...
type GoToPageResult<TEvaluateResult = any> = {
 ...
 */
export const GoToPage = async function<TEvaluateResult = any> (url: string, pageOptions: GoToPageOptions = {}): Promise<GoToPageResult<TEvaluateResult>> {
	const browser = await GetBrowser();
	const page = await browser.newPage();
	const result: GoToPageResult = {};
	const opts = {
		evaluateArgs: [],
		snapshots: false,
		content: false,
	...
	if (opts.evaluate) {
		if (opts.evaluateArgs && opts.evaluateArgs.length > 0) {
			result.evaluate = await page.evaluate(opts.evaluate, ...opts.evaluateArgs);
		}
		else {
			result.evaluate = await page.evaluate(opts.evaluate);
		}
	}
	if (opts.content) {
